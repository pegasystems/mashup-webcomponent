...
import {
  LitElement, property, html, customElement,
} from 'lit-element';
...
export class RichTextEditor extends LitElement {
    ...
    renderToolbar(command) {
      ...
      return html`
      ${commands.map((n) => html`
          ${n.values ? html` <select aria-label="${n.title}" tabindex=-1
        @change=${(e) => {
    e.preventDefault();
    ...
    : html` <button aria-label="${n.title}" tabindex=-1
