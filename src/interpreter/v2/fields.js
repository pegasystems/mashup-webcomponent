...
import { RichTextEditor } from '../../views/rich-text-editor';
...
const DisplayLabel = (data, path, type) => {
  ...
  if (type === 'field-checkbox') {
    if (data.label && data.label !== '') {
      return html`<legend class="field-caption">${unescapeHTML(data.label).replace('@L ', '')}</legend>`;
    }
    return null;
  }
  return html`
    ...
    for="${ifDefined(path)}">${unescapeHTML(data.label).replace('@L ', '')}
    </label>`
    : null}
  `;
};
...
const DisplayHelperText = (data, path) => {
  if (data.helperText && data.helperText !== '') {
    return html`<span id=${`${path}-info`} class="helper-text">${unescapeHTML(data.helperText).replace('@L ', '')}</span>`;
  }
  return null;
};
...
const GetPlaceholder = (data) => {
  if (data.placeholder && data.placeholder !== '') {
    return unescapeHTML(data.placeholder).replace('@L ', '');
  }
  return undefined;
};
export const AddWrapperDiv = (data, path, type, ComponentTemplate) => {
  ...
  return html`
    <div class="flex content-item field-item ${type}" ?disabled="${data.disabledstate}">
      ${DisplayLabel(data, path, type)}${ComponentTemplate}${DisplayHelperText(data, path)}
    </div>
  `;
};
...
export const Field = (data, path, isReadOnly, webcomp, context) => {
  ...
  if (typeof data.config.visibility === 'string') {
  ...
  switch (data.type) {
    ...
    case 'RichText':
      return AddWrapperDiv(data.config, path, 'field-richtext', RichText(data.config, path));
    case 'Checkbox':
    ...
    case 'Attachment':
      return AddWrapperDiv(data.config, path, 'field-attachment', AttachmentFile(data.config, path, webcomp));
    default:
      return null;
  }
};
 ...
 * RichText component
 */
const RichText = (data, path) => new RichTextEditor(unescapeHTML(data.displayvalue), data.reference, path);
...
const Checkbox = (data, path) => {
  ...
  />
  <label class=${iconrequired} for=${ifDefined(path)}>
  ${unescapeHTML(data.caption).replace('@L ', '')}
    </label>
`;
};
 ...
 */
const AttachmentFile = (data, path, webcomp) => {
  let files = [];
  if (data.displayvalue && data.displayvalue.pxResults && data.displayvalue.pxResults.length > 0) {
    files = data.displayvalue.pxResults;
  }
  return genAttachmentsList(undefined, files, webcomp.caseID, webcomp);
};
