...
export const AddWrapperDiv = (data, path, type, ComponentTemplate) => {
  ...
  return html`
    <div class="flex content-item field-item ${type}">
      ${type === 'field-checkbox' ? html`${ComponentTemplate}${DisplayLabel(data, path)}`
    : html`${DisplayLabel(data, path)}${ComponentTemplate}`}
    </div>
  `;
};
...
export const Field = (data, path, isReadOnly, webcomp, context) => {
  ...
  if (data.config.datasource && data.config.datasource.indexOf('@ASSOCIATED') === 0) {
    const obj = (context === '' ? webcomp.data.context_data.content : getValue(webcomp.data.context_data.content, context));
...
const DropDown = (data, path) => html`<select
  ...
  ?required="${data.required === true}">
    ...
    ${data.options.map(
    item => html`<option ?selected=${item.value === data.key} value='${item.key}'>${item.value}</option>`,
  )}
  </select>`;
