...
import { FieldPreProcessing } from '../../utils/fields-utils';
...
const DisplayLabel = (data, path, type) => {
  ...
  if (type === 'field-checkbox') {
    if (data.label && data.label !== '') {
      return html`<legend class="field-caption">${data.displaylabel}</legend>`;
    }
    return null;
  }
  return html`
...
const DisplayHelperText = (data, path) => {
  if (data.helperText && data.helperText !== '') {
    return html`<span id=${`${path}-info`} class="helper-text">${data.displayhelperText}</span>`;
  }
  return null;
};
