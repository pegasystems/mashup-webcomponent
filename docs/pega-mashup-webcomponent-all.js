...
`,Kt=(t,e,a,i)=>((t,e,a,i)=>{const n=document.createElement("div");n.setAttribute("role","alertdialog"),n.setAttribute("aria-modal","true"),n.setAttribute("aria-labelledby",t),n.setAttribute("aria-describedby",""),n.setAttribute("class","mashup-modal");const s=document.createElement("div");n.appendChild(s);const r=()=>{document.body.removeEventListener("click",o),n&&(n.previousElementSibling&&(n.previousElementSibling.removeAttribute("aria-hidden"),n.previousElementSibling.focus()),n.remove())},o=t=>{if(t){let e=t.target;if(e.download)return;if(t.path&&t.path.length>0?e=t.path[0]:t.originalTarget&&(e=t.originalTarget),e){const t=e.closest("button");if((null===t||"close"!==t.getAttribute("data-action-id"))&&!e.classList.contains("mashup-modal")&&null!==e.closest(".mashup-modal"))return}r()}};return O`
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" width="20px" height="20px" fill="currentColor">
  ...
  ${t.map(((t,n)=>{const s=`${e}-${n}`;if(t.layout){""!==t.layout.groupFormat.trim()&&"CENTER"!==t.layout.groupFormat||(t.layout.groupFormat="default");const e=t.layout.groupFormat.replace(/ /g,"_").toLowerCase(),n=`flex content layout-content-${e} content-${e}`;if(t.layout.view&&t.layout.view.groups)return O`
          <div class="${n}">${te(t.layout.view.groups,s,a,i)}</div>
        ...
        ${e}<div class="${n}">${te(t.layout.groups,s,a,i)}</div>
  ...
  ${Jt(t.layout,a)}
`)(t,0,a):ee(t,s,a):null}return t.field?t.field.control&&"pxAttachContent"===t.field.control.type?Kt("Upload file","Upload file","",i.displayAttachments):It(t.field,s,a):t.view&&t.view.groups?"pyAttachFieldOptional"===t.view.viewID&&"Embed-Attach-File"===t.view.appliesTo?Kt("Upload file","Upload file","",i.displayAttachments):te(t.view.groups,s,a,i):null}))}
`,ee=(t,e,a)=>O`
  ${Zt(t.layout)}
  <div class="rdl">
...
`,ae=(t,e,a)=>O`
  ${t.map(((t,i)=>{const n=`${e}-row${i}`;return t.groups?O`
      ...
      `:null}))}
`,ie=(t,e,a,i)=>{if(null==i)return null;let n;const s=`${Math.random().toString(36).substr(2,9)}`,r=document.createElement("ul");r.setAttribute("id",`menu-overlay-${s}`),r.setAttribute("role","menu"),r.setAttribute("aria-labelledby",`menubutton-${s}`),r.setAttribute("tabindex","-1");const o=()=>{document.body.removeEventListener("click",c),document.body.removeEventListener("keydown",l),r&&(r.previousElementSibling&&(r.previousElementSibling.setAttribute("aria-expanded","false"),r.previousElementSibling.focus()),r.remove())},l=t=>{38===t.keyCode?(n=n.previousElementSibling,null===n&&(n=r.lastElementChild),n.focus()):40===t.keyCode?(n=n.nextElementSibling,null===n&&(n=r.firstElementChild),n.focus()):27===t.keyCode||9===t.keyCode?o():13===t.keyCode&&(i(t),o())},c=t=>{if(t){let e=t.target;if(t.path&&t.path.length>0?e=t.path[0]:t.originalTarget&&(e=t.originalTarget),e&&e.nextElementSibling===r)return;e.parentNode===r&&i(t),o()}},d=t=>{if("keydown"===t.type&&40!==t.keyCode)return;if(t.preventDefault(),"blur"===t.type)return void c(t);const e=t.target;"BUTTON"!==e.tagName?(i(t),o()):null===e.nextElementSibling?(t.target.parentNode.appendChild(r),e.setAttribute("aria-expanded","true"),L(a(),r),n=e.nextElementSibling.firstElementChild,n.focus(),document.body.addEventListener("click",c),document.body.addEventListener("keydown",l)):o()};return O`
  ...
  `},ne=(t,e,a,i,n,s,r,o,l)=>{const c=O`${_t()}<span class='count-badge'>${n}</span>`;if(void 0===e.caseID&&a.content)return O`
    <div class="flex layout-content-inline_middle main-header">
 ...
 <h3>${t}</h3>`},se=(t,e,a,i,n)=>O`
  <div class="flex layout-content-stacked">${te(t,e,!1,n)}</div>
  ${((t,e)=>O`
  <div class="action-button-area">
...
`)(a)}
`,ce=(t,e)=>{const a=[],i=Object.entries(e.actions);for(const e of i)e[1].name!==t&&a.push(O`
...
${t.pxResults.map((t=>O`
  ...
  `))}
  </ul>`)(e)}else this.errorMsg=`Error ${e.errors[0].ID}: ${e.errors[0].message}`;this.clearLoadingIndicator(),n&&(n.disabled=!1,n.textContent="Save")}else if(e.access_token)this.token=e.access_token;else{const a=this.getRenderRoot().querySelector("#case-data");if("refreshcase"===t||"refreshassignment"===t||"refreshnew"===t){if(!a)return void console.error("Error: case data are not present");L("refreshnew"===t?le(e.creation_page.groups[0].layout.groups,"Obj","true"===this.bShowCancel?this.actionAreaCancel:null,this):se(e.view.groups,"Obj",this.actionAreaCancel,"true"===this.bShowSave?this.actionAreaSave:null,this),a)}else if("savecase"===t)this.sendExternalEvent(t),this.fetchData("case",{id:this.caseID,target:n}),""!==this.assignmentID&&this.fetchData("assignment",{id:this.assignmentID});else if("deleteattachment"===t||"attachments"===t)this.fetchData("attachments",{id:this.caseID,target:n});else if("uploadattachment"===t){let t=i.name.lastIndexOf(".");-1===t&&(t=i.name.length);const a=i.name.substring(t+1),s=[{type:"File",category:i.category,fileType:a,name:i.displayName,ID:e.ID}];return void this.sendData("attachments",{id:this.caseID,actionid:s,target:n})}e.ID&&(this.caseID=e.ID),e.nextAssignmentID?(a&&(L(Xt(),a),this.requestUpdate()),this.bShowNew=!1,this.assignmentID=e.nextAssignmentID,this.sendExternalEvent(t),this.fetchData("assignment",{id:this.assignmentID})):e.nextPageID&&(a&&(L(Xt(),a),this.requestUpdate()),this.sendExternalEvent(t),"Confirm"===e.nextPageID||"Review"===e.nextPageID?(this.bShowConfirm=!0,this.fetchData("view",{id:this.caseID,actionid:"pyCaseInformation"}),this.fetchData("case",{id:this.caseID})):this.fetchData("page",{id:this.caseID,actionid:e.nextPageID}))}this.requestUpdate()})).catch((t=>{this.genErrorMessage(t)}))}}const Ge=(t,e,a,i)=>O`
  <div class="flex layout-content-inline_middle main-header">
    <h2>${t} (${e})</h2>
...
`;G("pega-mashup")(class extends Qe{createRenderRoot(){return this.attachShadow({mode:"open"})}getRenderRoot(){return this.shadowRoot}render(){return O`
      ${ta()} ${this.displayContent()}
    `}}),G("pega-mashup-light")(class extends Qe{createRenderRoot(){return this}getRenderRoot(){return this}render(){return O`
      ${this.displayContent()}
    `}});const ea=(t,e,a,i)=>void 0===e?O`
      ${i}
    `:O`
...
`,la=(t,e)=>O`
  ...
  />
`,ca=(t,e)=>{let a=[{value:"true"},{value:"false"}];return t.options&&(a=t.options),O`
    ${a.map(((a,i)=>{const n=`rb-${e}-${i}`;return O`
        <div>
          <input
            data-ref="${t.reference}"
  ...
  ?required="${"true"===t.required}">
    ...
    <input
      ...
      value="${a}"
    />
  `},ua=(t,e)=>{let a=t.value;if(""!==a){let e=lt(a);e instanceof Date&&e.getTime()==e.getTime()?(e=new Date(e.getTime()+6e4*e.getTimezoneOffset()),a=`${e.getFullYear()}-${ot(e.getMonth()+1)}-${ot(e.getDate())}`):8===t.value.length&&(a=`${t.value.substring(0,4)}-${t.value.substring(4,6)}-${t.value.substring(6,8)}`)}return O`
    <input
      data-ref="${t.reference}"
      ?required="${"true"===t.required}"
      type="date"
      ...
      value="${a}"
    />
  `},pa=(t,e)=>{let a=t.value;if(""!==a){let e=lt(a);e instanceof Date&&e.getTime()==e.getTime()?(e=new Date(e.getTime()+6e4*e.getTimezoneOffset()),a=`${ot(e.getHours())}-${ot(e.getMinutes())}-${ot(e.getSeconds())}`):8===t.value.length&&(a=`${t.value.substring(9,10)}:${t.value.substring(10,11)}:${t.value.substring(11,12)}`)}return O`
    <input
      data-ref="${t.reference}"
      ?required="${"true"===t.required}"
      type="time"
      ...
      value="${a}"
    />
  `},ga=(t,e,a,i,n)=>{if(void 0===t)return null;if(!t.config||!t.config.template)return O`${t.map(((t,s)=>{const r=`${e}-${s}`;if("reference"===t.type&&"view"===t.config.type){let e="";return t.config.context&&(e=t.config.context),ga(i.data.uiResources.resources.views[t.config.name],r,a,i,""===n?e.substring(1):n+e)}return((t,e,a,i,n)=>{const s=t.config.value.replace("@P .","");if(t.config.reference=void 0===n||""===n?s:`${n}.${s}`,t.config.value=ht(i.casedata.content,t.config.reference),t.config.datasource&&0===t.config.datasource.indexOf("@ASSOCIATED")){const e=i.data.context_data?i.data.context_data.content:i.data.uiResources.context_data.caseInfo.content,a=""===n?e:ht(e,n);a&&(t.config.options=a.summary_of_associated_lists__[s])}if(t.config.options||(t.config.options=[]),t.config.value?t.config.value=`${t.config.value}`:t.config.value="",!0===a||"true"===t.config.readOnly)return ea(t.config,e,"field-text",aa(t.config,t.type,e));switch(t.type){case"Decimal":case"TextInput":return ea(t.config,e,"field-textinput",ia(t.config,e));case"Email":return ea(t.config,e,"field-emailinput",ra(t.config,e));case"Integer":return ea(t.config,e,"field-numberinput",na(t.config,e));case"Currency":return ea(t.config,e,"field-currencyinput",sa(t.config,e));case"RadioButtons":return ea(t.config,e,"field-radiogroup",ca(t.config,e));case"TextArea":return ea(t.config,e,"field-textarea",oa(t.config,e));case"Checkbox":return ea(t.config,e,"field-checkbox",la(t.config,e));case"Dropdown":return ea(t.config,e,"field-dropdown",da(t.config,e));case"DateTime":return ea(t.config,e,"field-datetime",ha(t.config,e));case"Date":return ea(t.config,e,"field-date",ua(t.config,e));case"Time":return ea(t.config,e,"field-time",pa(t.config,e));default:return null}})(t,r,a,i,n)}))}`;switch("TwoColumn"===t.config.template&&1===t.children.length&&(t.config.template="OneColumn"),t.config.template){case"TwoColumn":return O`
        ...
        `;default:return null}},ma=(t,e,a,i,n,s,r,o,l)=>{const c=O`${_t()}<span class='count-badge'>${n}</span>`;if(void 0===e.caseID&&a.content){const t=a.content.pyID.split(" ")[1];return O`
    <div class="flex layout-content-inline_middle main-header">
